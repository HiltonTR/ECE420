...
int main(int argc, char* argv[]) {
	...
	if (size == 1) {
		...
		for (k = 0; k < size - 1; ++k) {
			...
			#pragma omp parallel for private(temp,i,j) num_threads(thread_count) schedule(runtime)
			for (i = k + 1; i < size; ++i) {
		...
		// Jordan elim
		for (k = size - 1; k > 0; --k) {
			#pragma omp parallel for private(temp,i) num_threads(thread_count) schedule(runtime)
			for (i = 0; i < k; i++ ) {
				temp = Augmented[index[i]][k] / Augmented[index[k]][k];
		...
		// merge
		#pragma omp parallel for private(k) num_threads(thread_count) schedule(runtime)
		for (k=0; k< size; ++k) {
